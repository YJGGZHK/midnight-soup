...
- **保密**：普通接口不返回隐藏汤底；主动揭晓或成功破案后才返回。源码公开，阅读 `game.mjs` 会剧透。

```text
public/          页面、样式、交互及内嵌 SVG 插画
server.mjs       HTTP 服务、内存会话、访问校验
game.mjs        故事、评估请求和结果处理
test.mjs        Node 内置测试
...
测试覆盖：缺少 Key、推荐问题与自由提问调用模型、提示上限、汤底保密、会话恢复、输入校验、模型失败处理、概率阈值、跨站拒绝和局域网访问码。
...
[MIT](LICENSE)。项目代码和自带故事、SVG 插画按此许可证提供；第三方依赖保留各自许可证。项目不附带模型授权、API Key 或服务额度。
